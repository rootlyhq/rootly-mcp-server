...
- **AgentCat SDK upgraded to 2.1.0**: relaxed session-ID language, instruction text emitted only on the first response, and a low-footprint prepended session ID that avoids truncation when agents clip large responses. Telemetry-only and lazily imported; integration code unchanged.
