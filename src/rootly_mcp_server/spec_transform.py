...
_RESOURCE_ID_PATH_RE = re.compile(r"^/v\d+/(?P<resource>[a-z0-9_-]+)/\{id\}$")


# Collection segments whose singular drops "es" rather than just "s". English
# pluralization is ambiguous here — "causes" -> "cause" but "statuses" ->
# "status", and both end in "uses" — so the -es cases are listed explicitly
# rather than guessed. test_derived_aliases_cover_known_irregular_plurals keeps
# this honest against the bundled spec.
_ES_PLURAL_SINGULARS = {
    "addresses": "address",
    "processes": "process",
    "statuses": "status",
}
...
def _singularize(resource: str) -> str:
    """Best-effort singular form of a REST collection segment."""
    if resource.endswith("ies"):
        return resource[:-3] + "y"
    for plural, singular in _ES_PLURAL_SINGULARS.items():
        if resource == plural:
            return singular
        if resource.endswith(f"_{plural}"):
            return f"{resource[: -len(plural)]}{singular}"
    if resource.endswith("s"):
        return resource[:-1]
    return resource


def derive_resource_id_aliases(spec: dict[str, Any]) -> dict[str, str]:
    """Map each single-resource operation to its intuitive `<resource>_id` alias.

    Autogenerated tools for `/v1/teams/{id}` expose the path parameter as the
    generic ``id``, but callers reach for the semantic name — ``get_team`` is
    naturally called with ``team_id``. That argument is silently accepted (the
    OpenAPI tools do not enforce ``required``), the placeholder is never
    substituted, and the request fails late against ``/v1/teams/{id}``.

    Returns ``{snake_case_operation_id: alias_name}``. Derived from the spec so
    new endpoints are covered automatically. Callers must still drop entries
    whose alias collides with a real parameter on the registered tool — see
    ...
    for path, path_item in (spec.get("paths") or {}).items():
        if not isinstance(path_item, dict):
            continue
        match = _RESOURCE_ID_PATH_RE.match(path)
        if not match:
            continue
        alias = f"{_singularize(match.group('resource').replace('-', '_'))}_id"
        if alias == "id":
            continue
        for method, operation in path_item.items():
            ...
            operation_id = operation.get("operationId")
            if operation_id:
                aliases[to_snake_case(operation_id)] = alias
    return aliases


def filter_colliding_aliases(
    aliases: dict[str, str], tool_parameters: dict[str, set[str]]
) -> dict[str, dict[str, str]]:
    """Drop aliases that would shadow a real parameter, and shape for renaming.

    An alias is only safe when the tool actually exposes ``id`` and does *not*
    already expose the alias name. ``get_incident``/``update_incident`` are the
    real-world case: curated implementations shadow the autogen tools and
    legitimately take ``incident_id``, so aliasing ``incident_id -> id`` there
    would rewrite a valid argument into the wrong one.
    """
    safe: dict[str, dict[str, str]] = {}
    for operation_id, alias in aliases.items():
        params = tool_parameters.get(operation_id)
        if not params or "id" not in params or alias in params:
            continue
        safe[operation_id] = {alias: "id"}
    return safe
