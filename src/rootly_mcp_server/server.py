...
def _registered_tool_parameters(mcp: FastMCP) -> dict[str, set[str]]:
    """Return {tool_name: set(parameter names)} across curated and autogen tools."""
    parameters: dict[str, set[str]] = {}
    for provider in mcp.providers:
        components = getattr(provider, "_components", None) or {}
        autogen = getattr(provider, "_tools", None) or {}
        candidates: list[Any] = [
            component
            for key, component in components.items()
            if key.startswith("tool:") and getattr(component, "name", None)
        ]
        candidates.extend(autogen.values())
        for tool in candidates:
            ...
            if isinstance(schema, dict):
                parameters[name] = set((schema.get("properties") or {}).keys())
    return parameters
...
class ArgumentNormalizationMiddleware(fastmcp_middleware.Middleware):
    ...
    ``derived_renames`` carries spec-derived ``<resource>_id -> id`` aliases for
    autogenerated single-resource tools. It is populated after tool registration
    (collisions can only be detected against the final tool set), so the mapping
    is read at call time rather than captured at construction.
    """

    def __init__(self, derived_renames: dict[str, dict[str, str]] | None = None) -> None:
        # Mutated in place by create_rootly_mcp_server once tools are registered.
        self.derived_renames: dict[str, dict[str, str]] = derived_renames or {}
    ...
    async def on_call_tool(
        ...
        if args:
            static_renames = _ARGUMENT_RENAMES.get(tool)
            derived = self.derived_renames.get(tool)
            # Static entries win: a hand-curated mapping is more specific than
            # the generic <resource>_id rule.
            renames = (
                {**(derived or {}), **(static_renames or {})}
                if (static_renames or derived)
                else None
            )
            if renames:
...
def create_rootly_mcp_server(
    ...
    argument_normalization = ArgumentNormalizationMiddleware()
    mcp.add_middleware(argument_normalization)
    ...
    # Populate the <resource>_id -> id aliases now that the final tool set is
    # known. Callers reach for the semantic name (get_team(team_id=...)) while
    # autogen tools expose the generic `id`; because those tools do not enforce
    # `required`, the mismatch slips past validation and only fails when the
    # unsubstituted /v1/teams/{id} URL is rejected. Collisions are filtered
    # against real tool parameters, so curated tools that legitimately take
    # <resource>_id (get_incident, update_incident) are left alone.
    argument_normalization.derived_renames = spec_transform.filter_colliding_aliases(
        spec_transform.derive_resource_id_aliases(swagger_spec),
        _registered_tool_parameters(mcp),
    )
    logger.info(
        "Derived %d <resource>_id path-parameter alias(es)",
        len(argument_normalization.derived_renames),
    )
